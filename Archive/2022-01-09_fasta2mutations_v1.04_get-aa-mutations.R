...
for(i in 1:length(seq_files)){
  ...
  colnames(seq_sample) <- paste0(">", col_name[1])
  ...
  # Align the sample aa file to the reference aa file. WAIT THEY ARE ALREADY BASICALLY ALIGNED THIS CAN BE SKIPPED.
  # Perform a MSA using muscle.
  out_file <- paste0("./02_Align/", gsub(".fasta", "_align.fasta",gsub("./01_Sequence/","",temp_file)))
  command <- paste0("./muscle3.8.31_i86linux64 -profile -in1 temp_aa_ref.fasta -in2 temp_aa_sample.fasta -out temp_aa_align.fasta")
  ...
  seq <- ape::read.dna(out_file, format = "fasta")
  seq_aa <- ape::trans(seq)
  # Determine the SNP distances from the MSA.
  align_char_aa <- as.character(seq_aa)
  align_df_aa <- data.frame(align_char_aa)
  l <- length(align_df_aa)
  
  # Transpose the data frame so that it has 1 row for each position.
  align_df_aa <- as.data.frame(t(as.matrix(align_df_aa)))
  
  # Change the row index so that it allows it is the numbers and column names so that they are uniform.
  rownames(align_df_aa) <- seq(1:l)
  col_name <- colnames(align_df_aa)
  colnames(align_df_aa) <- c("seq1", "seq2")
  align_df_aa$seq1 <- as.character(align_df_aa$seq1)
  align_df_aa$seq2 <- as.character(align_df_aa$seq2)
