...
if(!file.exists("00_Results")){dir.create('00_Results')}
if(!file.exists("01_Sequence")){dir.create('01_Sequence')}
if(!file.exists("02_Align")){dir.create('02_Align')}
if(!file.exists("03_Mutation")){dir.create('mkdir 03_Mutation')}
...
fun <- function(x){if(grepl(">",x)){x <- paste0(x,'/NEWLINE/')}
  return(x)
}
fas2 <- data.frame(apply(fas1,1,fun)) # Add a marker so that we know where to add line spaces.
fas2 <- paste(unlist(fas2), collapse ="") # Remove all lines, this will make looping through the file easier.
# Add line break for the start of each fasta name.
fas2 <- gsub(">", "\n>",fas2) # Add a new line for fasta header
# Add line break for the start of each sequence. 
fas2 <- gsub("/NEWLINE/", "\n",fas2)

# Save the condensed fasta file to a temporary location. 
writeLines(fas2, single_line_file)
fas3 <- read.table(single_line_file)
colnames(fas3) <- c("fasta")

# Save this intermediate file.
file <- single_line_file
write.csv(fas3,file, row.names = F,  quote = F)

# Uncomment this for when starting with the genomes_all_single-line.fasta file.
file <- single_line_file
fas3 <- read.table(file, header = FALSE, sep = "", dec = ".")
names(fas3)[1] <- "fasta"
fas3$fasta <- as.character(fas3$fasta)

x <- 0
seq_num <- 1
for_file <- ""
# Add the necessary information to the reference file.
for(i in 1:length(fas3$fasta)){
  if(grepl(">",fas3$fasta[i]) == TRUE){
    ...
    ref$fasta[seq_num] <- fas3$fasta[i]
...
ref <- na.omit(ref) # when the ref file is made, there are too many lines. This makes it so that there are no extra lines.

# Make a file for each fasta to be aligned.
ref_seq <- read.csv(file_ref)
colnames(ref_seq) <- "fasta"
temp <- data.frame(matrix(NA, nrow = 2, ncol = 1))
colnames(temp) <- c("fasta")
for(i in 1:length(fas3$fasta)){ # Makes a fasta file that contains the wuhan sequenced that it is aligned with and the sample sequence. 
  # i <- 1
  if(grepl(">",fas3$fasta[i]) == TRUE){
    if(nchar(fas3$fasta[i + 1]) > 29000){
      ...
      temp$fasta[2] <- seq
      temp <- rbind(data.frame(fasta  = c(">NC_045512.2")),ref_seq,temp)
...
for(i in 1:length(seq_files)){
  ...
  if(ref$wgs[i] == "y"){
    ...
    ref$seq_name[i] <- gsub(">", "", ref$fasta[i])
    
    # Perform a MSA using muscle.
    out_file <- paste0("./02_Align/", gsub(".fasta", "_align.fasta",gsub("./01_Sequence/","",temp_file)))
    command <- paste0("mafft ", temp_file, " > ", out_file)
    
    unix(command)
    cat(paste0("Processing ", as.character(i), "/", as.character(length(ref$fasta)), " sequences"))
    ...
    # Handle insertions: nucleotides are inserted in the position immediately before the location where they are found.
    ...
    for(j in 1:length(align_df$POS)){
      ...
      # If insertion, condense it to the first occurrence of insertion. 
      if(align_df$seq1[j] == "-"){
        if(ins_new == "y"){
    ...
    # Remove the first and last group of lines that might results from sequencing (threshould is 50 continuous matching bp).
    cutoff <- 30
    if(length(align_mismatch$seq1) >= 1){
      align_mismatch$filter <- "n"    # Automatically set the filter to remove sequences.
      for(j in 1:length(align_mismatch$filter)){ # Only keep the nucleotides that were unambiguous. 
        if(align_mismatch$seq2[j] == "-" |
           align_mismatch$seq2[j] == "a" |
           align_mismatch$seq2[j] == "c" |
           align_mismatch$seq2[j] == "t" |
           align_mismatch$seq2[j] == "g"){
          align_mismatch$filter[j] <- "y" # If the nucleotide is not ambiguous, then it will not be set to pass the filtered.
        }
      ...
      # Remove the head end of mismatches. Usuaually the start of the sequence section contains many missing bases as a byrpoduct of sequencing. A cutoff value is used to determine when the majority of successful sequencing has occured. 
      for(j in 2:(length(align_mismatch$filter)+1)){
      ...
      if(grepl("del", align_mismatch$seq2[length(align_mismatch$filter)])){ # If the end is a string of deletions, it should be removed.
    ...
    ref$mutations[i] <- length(align_mismatch2$POS)
    ...
    if(length(align_mismatch4$POS)>0){ # If the sequence is identical to wuhan (could be low coverage) then do not perform the following functions.
      ...
      align_mismatch2b <- align_mismatch2 # These next two makes the true position number take into account insertions -- there will be a shift to an adjusted position number that reflects the number if there are insertions.
      align_mismatch2b$POS <- rownames(align_mismatch2b)
      for(j in 1:length(align_mismatch2b$seq1)){
        if(!grepl("del", align_mismatch2b$seq2[j])){
          if(!grepl("ins", align_mismatch2b$seq2[j])){
            seq_sample[as.numeric(align_mismatch2b$POS[j]),1] <- align_mismatch2b$seq2[j]
          }
        }
      }
      ...
      for(j in 1:length(cds$seqnames)){
        ...
        # Combine the reference and sample translation into one data frame.
  ...
  if (i%%10 == 0) {
    ...
    print(out_text, quote = FALSE)
    # Save an intermediate reference file as a csv. 
