...
file_original <- "2022-01-24_deer-genomes.fasta"
...
for(i in 1:length(seq_files)){
  ...
  if(ref$wgs[i] == "y"){
    ...
    seq_sample2 <- seq_sample[ , ! names(seq_sample) %in% c("seq2")]
    ...
    library(Biostrings)
    gb <- genabnkr::readGenBank('Wuhan-Hu-1.gb')
    ...
    opt$variantTableMajor <- bind_rows(lapply(split(opt$variantTableMajor, 1:nrow(opt$variantTableMajor)), function(x){
      ...
      v1 <- GRanges(seqnames = 'genome', ranges = IRanges(241, end = 241), strand = '+')
      o1 <- GenomicRanges::findOverlaps(v1, cds)
      
      v2 <- GRanges(seqnames = 'genome', ranges = IRanges(241, end = 241), strand = '+')
      ...
      #   
      #   genes <- paste0(hit2$gene, collapse = ', ')
        
        # Native gene AA sequence.
        orf1  <- as.character(translate(DNAString(substr(as.character(readFasta('nc_045512.2.fasta')@sread), start(hit1), end(hit1))), if.fuzzy.codon = 'solve'))
    ...
    for(j in 1:length(cds$seqnames)){
      ...
      temp_p <- subset(seq_sample, seq_sample$POS <= p_end)
      temp_p <- subset(temp_p, temp_p$POS >= p_start)
